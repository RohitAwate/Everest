 ...
 */
public class ROPCFlowProvider extends OAuth2FlowProvider {
    private ROPCController ropcController;

    ROPCFlowProvider(OAuth2FlowController controller) {
        super(controller);
        this.ropcController = (ROPCController) controller;
    }

    private void fetchAccessToken(RequestType type) throws Exception {
        HttpClient client = HttpClient.newHttpClient();
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(((ROPCState) ropcController.getState()).accessTokenURL))
                .header("Content-Type", MediaType.APPLICATION_FORM_URLENCODED)
                .POST(HttpRequest.BodyPublishers.ofString(generateRequestBody(type)))
                .build();

        HttpResponse<String> response = client.send(request, BodyHandlers.ofString());
        ROPCState state = (ROPCState) ropcController.getState();
    ...
    private ROPCToken parseTokenResponse(String response, String contentType)
        ...
        if (contentType.startsWith(MediaType.APPLICATION_JSON)) {
            ...
            token = new ROPCToken();
            String accessTokenURL = ((ROPCState) ropcController.getState()).accessTokenURL;
    ...
    private String generateRequestBody(RequestType type) {
        ...
        if (type == RequestType.NEW_TOKEN) {
            ...
            return String.format("grant_type=refresh_token&refresh_token=%s&client_id=%s&scope=%s",
                    token.getRefreshToken(), state.clientID, state.scope);
        }
    ...
    public OAuth2Token getAccessToken() throws Exception {
        ...
        ROPCToken token = (ROPCToken) state.accessToken;
        if (token == null || token.getRefreshToken().isBlank()) {
    ...
    public boolean isEnabled() {
        return ropcController.getState().enabled;
    }
    ...
    public String getAuthHeader() throws Exception {
        ...
        if (state.accessToken == null || state.accessToken.getAccessToken().isBlank()) {
            getAccessToken();
            ropcController.setAccessToken(state.accessToken);
        }
